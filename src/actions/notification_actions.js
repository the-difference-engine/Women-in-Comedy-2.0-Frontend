...
export const fetchNotifications = (userId) => {
    const request = axios({
        method: 'get',
        url: process.env.APP_API_URL + `notifications/${userId}`,
        headers: {"id": userId}
    });
    return (dispatch) => {
        request.then((data) => {
...
export const markNotificationsAsRead = (user_id)  => {
    const request = axios({
        method: 'get',
        url: process.env.APP_API_URL + `notifications/mark_all_read/${user_id}`,
        headers: {"id": user_id},
        data: {user_id}
    });
...
export const markOneAsRead = (user_id, id) => async dispatch => {
    ...
    const request = await axios({
        method: 'put',
        url: process.env.APP_API_URL + `notifications/mark_one/${user_id}`,
        data: { user_id, id }
    });
